...
import io.github.felseje.internal.cnpj.generation.AlphanumericCnpjGenerator;
import io.github.felseje.internal.cnpj.generation.NumericCnpjGenerator;
...
/**
 * Utility class for generating, validating, and formatting CNPJ values—both numeric and
 * alphanumeric (2026-ready).
 *
 * <p> Provides static methods for validating, formatting, normalizing, classifying, and generating
 * CNPJ values based on their format, in compliance with upcoming 2026 standards. </p>
 *
 * <p> This class is not intended to be instantiated and should only be used in a static context.
 * </p>
 *
 * @author felseje
 * @since 1.0.0-alpha
...
public final class CnpjUtils {
  ...
  /**
   * Generates a CNPJ string based on the specified {@link CnpjType}, with optional formatting
   * applied.
   *
   ...
   * @return {@code true} if the CNPJ is valid according to the specified type; {@code false}
   * otherwise
   ...
   *
   * @param cnpj the CNPJ string to validate
   * @return {@code true} if the CNPJ is valid according to its detected format; {@code false}
   * otherwise
  ...
  public static void validate(String cnpj, CnpjType type)
    ...
    requireTypeNonNull(type);
    StringUtils.requireNotBlank(cnpj, "The CNPJ must be not null or blank");
  ...
  public static void validate(String cnpj) throws IllegalArgumentException, InvalidCnpjException {
    StringUtils.requireNotBlank(cnpj, "The CNPJ cannot be null or blank");
   ...
   *
   * <p> The input must be normalized (i.e., stripped of any formatting characters and in uppercase
   * if applicable). </p>
   * <p>
   * Examples:
   * <pre>{@code
   ...
   *
   * <p> This method first normalizes the input using the configured {@link Normalizer}, then
   * applies formatting to produce a human-readable CNPJ. </p>
   *
   * <p>Examples:
   * <pre>{@code
   ...
   *
   * <p> This method ensures the input is not null or blank and removes all characters that are not
   * alphanumeric (e.g., dots, dashes, slashes, whitespace). </p>
   *
  ...
  public static String clear(String input) throws IllegalArgumentException {
    return NormalizerHolder.INSTANCE.sanitize(input);
  }
  ...
  /**
   * Holds a lazily-initialized singleton instance of {@link AlphanumericCnpjGenerator}.
   *
  ...
  private static final class AlphanumericGeneratorHolder {
    ...
    private static final AlphanumericCnpjGenerator INSTANCE = new AlphanumericCnpjGenerator();
  }

  /**
   * Holds a lazily-initialized singleton instance of {@link AlphanumericCnpjValidator}.
   *
  ...
  private static final class AlphanumericValidatorHolder {
    ...
    private static final AlphanumericCnpjValidator INSTANCE = new AlphanumericCnpjValidator(
        NormalizerHolder.INSTANCE);
  }

  /**
   * Holds a lazily-initialized singleton instance of {@link NumericCnpjGenerator}.
   *
  ...
  private static final class NumericGeneratorHolder {
    ...
    private static final NumericCnpjGenerator INSTANCE = new NumericCnpjGenerator();
  }

  /**
   * Holds a lazily-initialized singleton instance of {@link NumericCnpjValidator}.
   *
  ...
  private static final class NumericValidatorHolder {
    ...
    private static final NumericCnpjValidator INSTANCE = new NumericCnpjValidator(
        NormalizerHolder.INSTANCE);
  }
