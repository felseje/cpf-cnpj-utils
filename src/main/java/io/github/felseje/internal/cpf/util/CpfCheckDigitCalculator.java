 ...
 *
 * <p>It follows the official CPF validation algorithm defined by Receita Federal (Brazilian
 * IRS).</p>
 *
 * <p>Usage example:
 * <pre>{@code
...
public final class CpfCheckDigitCalculator {
  ...
  /**
   * Calculates a single check digit based on a given digit array. The calculation multiplies each
   * digit by its respective weight, computes the sum, and applies the modulo 11 rule.
   *
