...
public final class StringUtils {
  ...
  /**
   * Asserts that the provided string is not {@code null} or {@code blank}.
   *
   * <p>If the string is null or blank, an {@link IllegalArgumentException} is thrown
   * with the given error message. This method also validates that the {@code errorMessage} itself
   * is not null or blank.</p>
   *
   * @param string       the string to validate
   * @param errorMessage the message to include in the exception if validation fails
   * @throws IllegalArgumentException if the string is {@code null} or {@code blank}, or if the
   *                                  {@code errorMessage} is {@code null} or {@code blank}
   */
  public static void requireNotBlank(String string, String errorMessage)
